...
<script type="text/babel">
	...
	export default {
		...
		methods: {
			initEditor() {
				...
				wp.editor.initialize(this.editor_id, {
					...
					tinymce: {
						height: that.height,
						toolbar1:
							'bold,italic,bullist,numlist,link,blockquote,alignleft,aligncenter,alignright,strikethrough,forecolor,codeformat,undo,redo',
						setup(ed) {
							ed.on('init', (ed) => {
								tinyMCE
									.get(that.editor_id)
									.setContent(that.value);
			...
			updateCursorPos() {
				var cursorPos = jQuery('.wp_vue_editor_plain').prop(
					'selectionStart'
				);
