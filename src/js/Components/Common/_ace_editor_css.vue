<template>
	<div v-loading="loading" element-loading-text="Loading Editor...">
		<div class="ace_container">
			<div class="ninja_custom_css_editor" id="ninja_custom_css">
				{{ value }}
			</div>
		</div>
		<div class="editor_errors" :class="'ninja_' + mode + '_errors'">
			<span
				v-show="editorError"
				style="text-align: right; display: inline-block; color: #ff7171; float: right"
				>{{ editorError }}</span
			>
		</div>
	</div>
...
<script type="text/babel">
	export default {
		...
		data() {
			return {
				ace_path: window.authorBioAdmin.ace_path_url,
				editorError: '',
				loading: true
			};
		},
